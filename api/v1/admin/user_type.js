...
router.get('/:id', async (req, res, next) => {
  try {
    const { id } = req.params;
    const category = await userType.findById(id);
    res.json(category);
  } catch (err) {
    next(err);
  }
...
router.post('/', async (req, res, next) => {
  try {
    const category = new userType(req.body);
...
router.put('/:id', async (req, res, next) => {
  try {
    const { id } = req.params;
    const updateResult = await userType.findByIdAndUpdate(id, req.body);
...
router.delete('/:id', async (req, res, next) => {
  try {
    const { id } = req.params;
    const delResult = await userType.findByIdAndDelete(id);
    res.json(delResult);
  } catch (err) {
    next(err);
  }
