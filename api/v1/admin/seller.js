...
router.get('/', async (req, res, next) => {
  ...
  const totalCount = await Seller.find(query).count();
  const users = await Seller.find(query).sort({ createdAt: -1 })
    .limit(per)
    .skip(per * (page - 1));
  res.json({
    totalCount,
...
router.post('/', async (req, res, next) => {
  const userExist = await Seller.count({ userName: req.body.userName });
  if (userExist > 0) {
    res.json({
      code: 'error',
...
router.put('/:id', async (req, res, next) => {
  try {
    ...
    const updateResult = await Seller.findByIdAndUpdate(id, req.body);
...
router.put('/reset_pwd/:id', async (req, res, next) => {
  try {
    ...
    const updateResult = await Seller.findByIdAndUpdate(id, {
      password: pwd
    })
    res.json(updateResult);
  } catch (err) {
...
router.delete('/:id', async (req, res, next) => {
  try {
    const { id } = req.params;
    const delResult = await Seller.findByIdAndDelete(id);
    res.json(delResult);
  } catch (err) {
    next(err);
  }
